...
{
    public partial class FormHattyusakiMstcs : Form
    {
        public FormHattyusakiMstcs()
        {
            InitializeComponent();
        }
        //SQLサーバー接続文字
        ...
        private void ListIni()
        {
            ...
            try
            {
                //接続成功
                //すべてのレコードをコード昇順で出力
                string sqlstr = "select * from 発注先マスタ ORDER BY 発注先コード";
                ...
                while (sdr.Read() == true)
                {
                    ...
                    string code = (string)sdr["発注先コード"];
                    string name = (string)sdr["発注先名"];
        ...
        private void RecordDisp(string codestr)
        {
            ...
            if (codestr != "")
            {
                ...
                try
                {
                    ...
                    //得意先コードで検索
                    string sqlstr = "select * from 発注先マスタ where 発注先コード=" + codestr;
                    SqlCommand com = new SqlCommand(sqlstr, con);
                    SqlDataReader sdr = com.ExecuteReader();
        ...
        private void FormHattyusakiMstcs_Load(object sender, EventArgs e)
        {
            //リスト表示
            ListIni();
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (sdr.Read() == true)
                {
                    //同一コードが見つかった場合
                    //更新
                    saveSqlStr = "UPDATE 発注先マスタ " +
                        "SET" +
                        " 発注先コード='" + textBoxTokuisakiCode.Text.Trim() +
                        "' ,発注先名=N'" + textBoxTokuisakiMei.Text.Trim() +
                        "' ,TEL='" + textBoxTel.Text.Trim() +
                        "' ,FAX='" + textBoxFax.Text.Trim() +
                        "' where 発注先コード  = '" + codestr + "'";
                }
                else
                {
                    //同一コードが見つからなかった場合
                    //新規
                    saveSqlStr = "INSERT INTO 発注先マスタ (" +
                            "発注先コード, 発注先名, TEL, FAX" +
                            ") VALUES ('" +
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                labelID.Text = "";
                //メッセージボックス表示
                MessageBox.Show(msgstr, "発注先マスタ　削除");
            }
        }
